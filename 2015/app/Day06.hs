...
startingGrid :: [[Bool]]
startingGrid = [[False | _ <- [0..999]] | _ <- [0..999]]

startingGrid2 :: [[Int]]
startingGrid2 = [[0 | _ <- [0..999]] | _ <- [0..999]]
...
deriveInstructions input =
        let all_submatches = map tail ( input =~ instructionRegex :: [[String]] )
...
flatCount :: [[Bool]] -> Int
flatCount items = length (filter id (concat items))
...
day06main = do
    ...
    putStrLn $ "Total Brightness on: " ++ show (flatSum brightGrid)
