...
isNice (NN vowels hasDouble) = (traceShowId vowels) >= 3 && hasDouble
...
firstCheck (a:b:r) = if bookEnded then True else firstCheck (b:r)
    where
...
findBookend (a:b) (x:y:r) = if (a:b) == (y:[x]) then True else findBookend (a:b) (y:r)
